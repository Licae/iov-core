...
def main() -> int:
    ...
    if not host:
        return finish("BLOCKED", "资产未配置连接地址，无法执行 Telnet 访问测试。", steps, 0)
    ...
    steps.append(
        step(
            ...
            output=probe_output,
            security_assessment="Telnet 服务已暴露，访问控制失败。" if probe_ok else "Telnet 服务未暴露，访问控制符合预期。",
        )
    )

    if probe_ok:
        return finish("FAILED", f"目标 {host}:{port} 暴露了 Telnet 服务，访问控制失败。", steps, max(1, int(time.time() - started_at)))
    return finish("PASSED", f"目标 {host}:{port} 未暴露 Telnet 服务，访问控制符合预期。", steps, max(1, int(time.time() - started_at)))
